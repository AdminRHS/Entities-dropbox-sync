...
.input:focus {
  outline: none;
  border-color: var(--color-primary-default); /* #2563EB */
...
.input.error {
  border-color: var(--color-delete-default);  /* #DC2626 */
}
...
.modal-overlay {
  ...
  align-items: center;
  justify-content: center;
  z-index: var(--z-backdrop);                /* 1040 from Design System */
  animation: fadeIn var(--transition-fast) ease-in-out; /* 150ms */
}
...
/* Modal Content (from Design System) */
.modal {
  background: var(--bg-paper);               /* #ffffff */
  border-radius: 12px;                       /* from Design System */
  box-shadow: var(--shadow-heavy);           /* 0 10px 30px rgba(0,0,0,0.15) */
  max-width: 90vw;
  max-height: 90vh;
  overflow: hidden;
  z-index: var(--z-modal);                   /* 1050 from Design System */
  animation: slideUp var(--transition-normal) ease-out; /* 300ms */
}
...
```css
/* Filter Buttons (as on reference) */
.filter-group {
  display: flex;
  align-items: center;
  gap: var(--space-3);
...
```css
/* Custom Scrollbar (as on reference) */
::-webkit-scrollbar {
  width: 8px;
  height: 8px;
}
...
### ✨ ANIMATIONS & TRANSITIONS (from Design System)

**Motion & Transitions (from Design System):**

| Duration | Value | Usage |
|----------|-------|-------|
| Fast | 150ms | Micro-interactions, button press |
| Normal | 300ms | Standard transitions, hover effects |
| Slow | 500ms | Page transitions, animations |

**Easing:** ease-in-out for entrance/exit transitions
...
### 📐 Z-INDEX LAYERS (from Design System)
...
| Backdrop | 1040 | Modal backgrounds |
| Modal | 1050 | Modal windows |
| Popover | 1060 | Popover elements |
...
/* Wave Animation (for voice search, as on reference) */
...
| Large Desktop | 1200px - 1439px | Large desktops |
| Extra Large | 1440px+ | Extra large screens |
...
#### Search Results Modal (key component)
...
2. **Create Search Task Modal**
   - Form fields:
     ...
     - Department (dropdown)
     - Topic (text input)
     - Search Type (dropdown with prompts)
...
- **APIs:**
  ...
  - Dropbox API (for files)
  - YouTube Data API v3 (for metadata)
  - OpenAI API (optional, for AI search)
...
1. Setup Tailwind CSS with all CSS variables
2. Create base components (Button, Badge, Input, Modal, Card)
3. Configure dark mode toggle
4. Create sidebar navigation layout
5. Responsive breakpoints

### Phase 2: Search Queue (2-3 days)
1. Search Queue Dashboard
2. Create Search Task Modal
3. Search Results Modal (key component!)
...
### Design
- ✅ Matches reference: https://adminrhs.github.io/Video-catalog/
- ✅ Dark/Light mode works correctly
- ✅ All colors from department colors palette are used
- ✅ Smooth transitions (300ms) on all elements
- ✅ Responsive on all devices
- ✅ Custom scrollbar styling applied

### Functionality
- ✅ Search Queue: creation, execution, results
- ✅ Video Queue: addition, prioritization, statuses
- ✅ Modals: opening, closing, overlay
- ✅ API integration: all endpoints work
- ✅ YouTube API: metadata loads
- ✅ Priority Calculator: correct calculation 0-100
...
1. **Strictly follow the reference design** - colors, spacing, typography, animations
2. **Use all CSS variables** - do not hardcode values
3. **Dark mode is mandatory** - toggle must work everywhere
4. **Search Results Modal - key component** - maximum attention
5. **Priority system is important** - stars must display correctly
6. **Responsive is mandatory** - mobile, tablet, desktop
7. **Smooth animations** - all transitions 200-300ms
8. **Department colors** - each module has its own color
9. **shadcn/ui** - use their components as base
10. **TypeScript** - strict typing everywhere
...
*This prompt contains a complete description of design, styles, components and functionality for generating RESEARCHES 2 application with design matching https://adminrhs.github.io/Video-catalog/*
